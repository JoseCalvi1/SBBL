...
<div class="container-fluid blade-container">
    ...
    <form method="GET" action="{{ route('database.parts') }}" class="filter-container">
    ...
    <div class="system-row">
        @foreach ($blades as $blade)
            <div class="blade-item">
                <a href="{{ route('database.showBlade', $blade->id) }}">
                    <img src="{{ $blade->tarjeta }}" alt="{{ $blade->nombre_takara }}">
                </a>
            </div>
        @endforeach
    </div>
    ...
    <div class="row" style="text-align: center;">
        <h2 style="color: white;">RATCHETS</h2>
    </div>
    <div class="system-row">
        @foreach ($ratchets as $ratchet)
            <div class="blade-item">
                <a href="{{ route('database.showRatchet', $ratchet->id) }}">
                    <img src="{{ $ratchet->tarjeta }}" alt="{{ $ratchet->nombre }}">
                </a>
            </div>
        @endforeach
    </div>
    ...
    <div class="row" style="text-align: center;">
        <h2 style="color: white;">Bits</h2>
    </div>
    <div class="system-row">
        @foreach ($bits as $bit)
            <div class="blade-item">
                <a href="{{ route('database.showBit', $bit->id) }}">
                    <img src="{{ $bit->tarjeta }}" alt="{{ $bit->nombre }}">
                </a>
            </div>
        @endforeach
    </div>
