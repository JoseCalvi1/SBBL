...
<h2 class="text-center mb-5 text-white">Editar duelo de equipos</h2>

<div class="row justify-content-center mt-5">
    <div class="col-md-8">
        <form method="POST" action="{{ route('teams_versus.update', ['duel' => $duel->id]) }}" enctype="multipart/form-data" novalidate style="color: white">
            @csrf
            @method('PUT')
            @if(session('error'))
            ...
            <div class="form-group">
                ...
                <select name="team_id_1" id="team_id_1" class="form-control select2 @error('team_id_1') is-invalid @enderror">
                    @foreach ($teams as $team)
                        <option value="{{ $team->id }}" {{ $duel->versus_1->id == $team->id ? 'selected' : '' }}>{{ $team->name }}</option>
                    @endforeach
                </select>
                @error('team_id_1')
            ...
            <div class="form-group">
                ...
                <select name="team_id_2" id="team_id_2" class="form-control select2 @error('team_id_2') is-invalid @enderror">
                    @foreach ($teams as $team)
                        <option value="{{ $team->id }}" {{ $duel->versus_2->id == $team->id ? 'selected' : '' }}>{{ $team->name }}</option>
                    @endforeach
                </select>
                @error('team_id_2')
                    <span class="invalid-feedback d-block" role="alert">
            ...
            <div class="form-group">
                ...
                <select name="modalidad" id="modalidad" class="form-control @error('modalidad') is-invalid @enderror">
                    <option value="beybladex" {{ $duel->modalidad == 'beybladex' ? 'selected' : '' }}>Beyblade X</option>
                </select>
                @error('modalidad')
                    <span class="invalid-feedback d-block" role="alert">
                        <strong>{{$message}}</strong>
            ...
            <div class="form-group">
                <input type="submit" class="btn btn-primary" value="Actualizar duelo">
            </div>
        </form>
    </div>
</div>
